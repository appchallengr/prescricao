...
@Component({
  selector: 'app-item-abreviacao-historico',
  templateUrl: './item-abreviacao-historico.component.html',
  styleUrls: ['./item-abreviacao-historico.component.css']
})
export class ItemAbreviacaoHistoricoComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ItemAbreviacaoHistoricoComponent>,
  ...
  getList() {
    ...
    this.itemAbreviacaoService.getHistoric(this.body, this.searchValue)
      .subscribe(
        (data: any) => {
  ...
  exportExcel(result){
    const dataExcel = [];
    this.itemAbreviacaoService.getExcelHistorico(result.id).subscribe((data: any) => {
      console.log("result",data)
      dataExcel.push({
        'Propriedade': 'Nome do Item',
